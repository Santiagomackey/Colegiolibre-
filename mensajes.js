(function () {
  ...
  async function hydrateConversationTrust(conversation) {
    ...
    if (conversation.product.status === "reserved") {
      const { data: reservation } = await window.colegioLibreSupabase
        .from("transactions")
        .select(
          "id, buyer_id, seller_id, status, reserved_at, cancelled_at, cancelled_by"
        )
      ...
      const isReservedBuyer =
        reservation?.buyer_id === state.currentUser.id &&
        conversation.product.reserved_for === state.currentUser.id;

      panel.innerHTML = `
        <div class="trust-status-card">
          <strong>${
            isReservedBuyer ? "Este producto está reservado para vos" : "Reserva registrada"
          }</strong>
          <span>
            ${
              reservation?.reserved_at
                ? `Reserva creada ${escapeHtml(
                    formatRelativeDate(reservation.reserved_at).toLowerCase()
                  )}. `
                : ""
            }
            ColegioLibre no procesa pagos. Coordinen el intercambio en un
            lugar seguro y revisen el producto antes de pagar.
          </span>
          ${
            isReservedBuyer
              ? `
      ...
      panel
        .querySelector("[data-cancel-own-reservation]")
        ?.addEventListener("click", (event) =>
          cancelOwnReservation(conversation, event.currentTarget)
        );
      return;
    }
  ...
  async function cancelOwnReservation(conversation, button) {
    if (
      conversation.buyer_id !== state.currentUser.id ||
      conversation.product?.reserved_for !== state.currentUser.id
    ) {
      showToast("Esta reserva no está asociada a tu cuenta.");
      return;
    }

    if (
      !window.confirm(
        "¿Querés cancelar tu reserva? El producto volverá a estar disponible para otras personas."
      )
    ) {
      return;
    }
