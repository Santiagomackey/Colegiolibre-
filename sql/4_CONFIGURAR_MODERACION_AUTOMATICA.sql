...
begin
  if not public.is_admin_account(auth.uid()) then
    raise exception 'Solo un administrador puede revisar publicaciones.';
  end if;
  ...
  if char_length(btrim(coalesce(moderator_note, ''))) < 2 then
    raise exception 'Agregá un motivo breve.';
  end if;
  ...
  resulting_status := case when clean_decision = 'approved' then 'available' else 'paused' end;

  update public.products
  set
    moderation_status = clean_decision,
    moderation_reason = left(btrim(moderator_note), 800),
  ...
  set
    decision = clean_decision,
    reason = left(btrim(moderator_note), 800),
    source = 'admin',
    confidence = 1,
...
drop policy if exists "Owners and admins read product moderation reviews"
on public.product_moderation_reviews;

create policy "Owners and admins read product moderation reviews"
on public.product_moderation_reviews
for select
to authenticated
using (
...
revoke all on public.product_moderation_reviews from public, anon;
revoke insert, update, delete on public.product_moderation_reviews from authenticated;
grant select on public.product_moderation_reviews to authenticated;
...
revoke all on function public.review_product_moderation(uuid, text, text)
from public, anon;
grant execute on function public.review_product_moderation(uuid, text, text)
to authenticated;
...
select
  to_regclass('public.product_moderation_reviews') is not null
    as revisiones_configuradas,
  exists (
    select 1
  ...
  ) as productos_configurados,
  exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'profiles'
      and column_name = 'moderation_strikes'
  ) as sanciones_configuradas,
  (
    select count(*) = 1
    from information_schema.triggers
    where trigger_schema = 'public'
      and event_object_table = 'products'
      and trigger_name = 'products_00_queue_for_moderation'
  ) as cola_configurada;
